...
function SendNotificationToPlayer(player, imageName, headerMsg, detailsMsg, message) {
  alt.emitClient(player, 'freeroam:sendNotification', imageName, headerMsg, detailsMsg, message);
}
...
chat.registerCmd("help", (player, args) => {
 ...
 // chat.send(player, "{ff0000}= {34abeb}/veh {40eb34}(model)   {ffffff} Spawn a Vehicle");
  ...
  chat.send(player, "{ff0000}= {34abeb}/hp {40eb34}(amount)   {ffffff} Set your health to amount");
...
chat.registerCmd("hp", (player, args) => {
  if (args.length === 0) {
    chat.send(player, "Usage: /hp (amount)");
    return;
  }
...
chat.registerCmd("notif", (player, args) => {
  if (args.length === 0) {
    chat.send(player, "Usage: /notif (message)");
    return;
  }
  SendNotificationToPlayer(player, 'CHAR_AMMUNATION', 'Admin', 'Слушай сюда', args[0]);
});

chat.registerCmd("st", (player, args) => {
  if (args.length === 0) {
    chat.send(player, "Usage: /st (amount)");
    return;
  }
  player.stamina = args[0];
});
